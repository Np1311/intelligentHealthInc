...
class RadiologyRecord(models.Model):
    ...
    def area_data(self):
        ...
        if self.filtered_records is not None:
            area_data = list(self.filtered_records.values_list('area', flat=True))
    ...
    def nationality_data(self):
        if self.filtered_records is not None:
            nationality_data = list(self.filtered_records.values_list('nationality', flat=True))
    ...
    def age_data(self):
        if self.filtered_records is not None:
            age_data = list(self.filtered_records.values_list('age', flat=True))
            age_ranges = [(0, 10), (11, 20), (21, 30), (31, 40), (41, 50), (51, 60), (61, float('inf'))]
            age_labels = ['<10', '11-20', '21-30', '31-40', '41-50', '51-60', '>60']
        ...
        if self.filtered_records is not None:
            ...
            for visit_time in visit_data:
                ...
                for week_label, week_start in visit_counts.items():
                    week_start = datetime.strptime(week_label.split(' ')[0], '%d-%m-%Y').date()
                    week_end = datetime.strptime(week_label.split(' ')[-1], '%d-%m-%Y').date()
